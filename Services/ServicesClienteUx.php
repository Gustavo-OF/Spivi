        ...
        if(!empty($pesquisa)){
            if(is_numeric($pesquisa)){
                if(strlen($pesquisa) == 11){
                    $retorno = $this->database->select("TB_PESSOAS","COD_ALUNO, NOME, CPF, ID_TIPO_PESSOAS",[],"CPF = ?",[$pesquisa]);
                }else{
                    $retorno = $this->database->select("TB_PESSOAS","COD_ALUNO, NOME, CPF, ID_TIPO_PESSOAS",[],"COD_ALUNO = ?",[$pesquisa]);
                }
            }else{
                $retorno = $this->database->select("TB_PESSOAS","COD_ALUNO, NOME, CPF, ID_TIPO_PESSOAS",[],"NOME LIKE ? AND COD_UNIDADE = ?",["%".$pesquisa."%",$codUnidade]);
            }
        }
